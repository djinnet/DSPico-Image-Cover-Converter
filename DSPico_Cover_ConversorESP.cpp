...
const std::wstring txtFullTitle = L"Conversor Cover DSPico v9.1 - Rikisoft 2026";
...
DWORD WINAPI ThreadMotor(LPVOID lpParam) {
    ...
    if (shouldExit) { SaveConfig(); PostMessage(hMain, WM_CLOSE, 0, 0); }
        ...
        SetWindowTextW(hBtnStop, L"CONTINUAR"); EnableWindow(hBtnStop, TRUE);
        AddLog(L"Quedan: " + std::to_wstring(nImg - (nDone + nSkipped))); SetWindowTextW(hBtnStart, L"REINICIAR");
    } else { 
        lastIndex = 0; EnableWindow(hBtnStop, FALSE); SetWindowTextW(hBtnStop, L"DETENER"); 
        AddLog(L"Finalizado."); PlayRes("SND_FINISH"); SetWindowTextW(hBtnStart, L"INICIAR");
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    switch (msg) {
        ...
        case WM_PAINT: {
            ...
            TextOutW(hdcMem, 25, 40, L"COVERS", 6); TextOutW(hdcMem, 25, 83, L"DSPICO (SDPico_pico\\covers\\user)", 32);
            TextOutW(hdcMem, 25, 126, L"ROMS (SDPico\\Directorio de Roms)", 32);
            int midY = 245; wchar_t sT[32], sH[32], sS[32], sC[32];
            swprintf(sT, 32, L"Total: %d", nImg); swprintf(sH, 32, L"Hechas: %d", nDone); swprintf(sS, 32, L"Omitidas: %d", nSkipped); swprintf(sC, 32, L"Limp: %d", nCleaned);
        ...
        case WM_COMMAND:
            if (LOWORD(wp) == ID_BTN_START) {
                ...
                if (!PathFileExistsA(cS) || !PathFileExistsA(cD) || strlen(cS) < 3) { debugLog.clear(); AddLog(L"ERR: Rutas"); PlayRes("SND_QUITAR"); return 0; }
                ...
                nImg = (int)lista.size(); if (nImg == 0) { debugLog.clear(); AddLog(L"Origen vacío"); return 0; }
                ...
                if (!force && !limp && yaEx == nImg) { nSkipped = yaEx; nDone = 0; debugLog.clear(); AddLog(L"Todo al día"); PlayRes("SND_SELECT"); SetWindowTextW(hBtnStart, L"INICIAR"); return 0; }
                debugLog.clear(); nCleaned = 0; nDone = 0; nSkipped = 0; lastIndex = 0; isAbort = false; isWorking = true; shouldExit = false;
                if (limp) { if (strlen(cR) >= 3 && PathFileExistsA(cR)) EjecutarLimpieza(cD, cR); else { AddLog(L"ERR: Ruta ROMS"); isWorking = false; return 0; } }
                SetWindowTextW(hBtnStart, L"INICIAR"); SetWindowTextW(hBtnStop, L"DETENER"); EnableWindow(hBtnStop, TRUE);
                AddLog(L"Procesando..."); CreateThread(0,0,ThreadMotor,0,0,0);
            }
            if (LOWORD(wp) == ID_BTN_STOP) {
                debugLog.clear();
                if(isWorking) { isAbort = true; SetWindowTextW(hBtnStart, L"REINICIAR"); }
                else if (isAbort && lastIndex > 0) { 
                    isAbort = false; isWorking = true; AddLog(L"Quedan: " + std::to_wstring(nImg - (nDone + nSkipped)));
                    SetWindowTextW(hBtnStart, L"REINICIAR"); SetWindowTextW(hBtnStop, L"DETENER"); CreateThread(0,0,ThreadMotor,0,0,0); 
                }
            }
            if (LOWORD(wp) == ID_BTN_CLOSE) { 
                if (isWorking) { 
                    shouldExit = true; isAbort = true; AddLog(L"Cerrando..."); 
        ...
        case WM_CREATE: {
            ...
            hChk1 = CreateWindowW(L"BUTTON", L"Sobreescribir", WS_VISIBLE|WS_CHILD|BS_AUTOCHECKBOX, 25, 175, 105, 20, hwnd, (HMENU)ID_CHK_FORCE, 0, 0);
            hChk2 = CreateWindowW(L"BUTTON", L"Abrir al final", WS_VISIBLE|WS_CHILD|BS_AUTOCHECKBOX, 165, 175, 105, 20, hwnd, (HMENU)ID_CHK_OPEN, 0, 0);
            hChk3 = CreateWindowW(L"BUTTON", L"Limpiar covers", WS_VISIBLE|WS_CHILD|BS_AUTOCHECKBOX, 295, 175, 105, 20, hwnd, (HMENU)ID_CHK_CLEAN, 0, 0);
            hBtnStart = CreateWindowW(L"BUTTON", L"INICIAR", WS_VISIBLE|WS_CHILD|BS_OWNERDRAW, 25, 340, 70, 28, hwnd, (HMENU)ID_BTN_START, 0, 0);
            hBtnStop  = CreateWindowW(L"BUTTON", L"DETENER", WS_VISIBLE|WS_CHILD|BS_OWNERDRAW|WS_DISABLED, 140, 340, 130, 28, hwnd, (HMENU)ID_BTN_STOP, 0, 0);
            hBtnClose = CreateWindowW(L"BUTTON", L"CERRAR", WS_VISIBLE|WS_CHILD|BS_OWNERDRAW, 315, 340, 70, 28, hwnd, (HMENU)ID_BTN_CLOSE, 0, 0);
            ...
            if (SearchPathA(NULL, "magick.exe", NULL, MAX_PATH, dummy, NULL) == 0) {
                ...
                int res = MessageBoxW(hwnd, 
                    L"No se ha encontrado ImageMagick (magick.exe) en el PATH.\n\n"
                    L"¿Deseas abrir la web oficial para descargarlo?\n"
                    L"(La aplicación se cerrará para permitir la instalación).", 
                    L"Herramienta no encontrada", MB_YESNO | MB_ICONWARNING);
                
                if (res == IDYES) {
                    ShellExecuteW(NULL, L"open", L"https://imagemagick.org/script/download.php#windows", NULL, NULL, SW_SHOWNORMAL);
                    PostQuitMessage(0); // CERRAMOS EL PROGRAMA
                    return 0;
                }
